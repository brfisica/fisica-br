# Talleres Interactivos de Física

Simulaciones interactivas para los cursos de Física

**Autores:** Juan David Betancur Ríos · Nora Yamile Rojas Cataño
**Semestre:** 2026-2

Cada taller es un cuaderno interactivo (Jupyter/Voilà) con esquemas del dispositivo,
deslizadores para experimentar, soluciones ocultas tras un botón y cuestionarios
autoevaluables. Los estudiantes los abren en el navegador mediante Binder, sin instalar
nada ni requerir cuenta.

## Talleres disponibles

| Taller | Curso | Enlace (Binder) |
|--------|-------|-----------------|
| Ley de Gauss | Electricidad y Magnetismo | [Abrir](https://mybinder.org/v2/gh/brfisica/Ley-de-Gauss/HEAD?urlpath=voila%2Frender%2FTaller_Ley_de_Gauss_Interactivo_v2+%281%29.ipynb) |
| Potencial Eléctrico | Electricidad y Magnetismo | [Abrir](https://mybinder.org/v2/gh/brfisica/Ley-de-Gauss/HEAD?urlpath=voila%2Frender%2FTaller_Potencial_Electrico.ipynb) |
| Movimiento de Proyectiles | Física Mecánica | [Abrir](https://mybinder.org/v2/gh/brfisica/Ley-de-Gauss/HEAD?urlpath=voila%2Frender%2FTaller_Proyectiles.ipynb) |
| Movimiento bajo la Gravedad | Física Mecánica | [Abrir](https://mybinder.org/v2/gh/brfisica/Ley-de-Gauss/HEAD?urlpath=voila%2Frender%2FTaller_Caida_Libre.ipynb) |

> La primera carga puede tardar 1–2 minutos mientras Binder construye el entorno.

## Licencia

© 2026 Juan David Betancur Ríos y Nora Yamile Rojas Cataño.

Este material está licenciado bajo
**Creative Commons Atribución-NoComercial-CompartirIgual 4.0 Internacional
(CC BY-NC-SA 4.0)**. Puede usarse y adaptarse con fines educativos citando a los
autores, sin fines comerciales y compartiendo bajo la misma licencia.
Ver el archivo [LICENSE](LICENSE) para los detalles.

*Material elaborado con apoyo de herramientas de inteligencia artificial, bajo la
supervisión y criterio pedagógico de los autores.*
